...
- **1.4 Typed dependency resolution.** Give `DependencyContainer.resolve()` a real generic signature — a `resolve(self, key: type[T]) -> T` / `-> Any` `@overload` pair — so resolving by service class is statically typed. This erased **29 of the ~72 catalogued mypy errors** (`src/ui/main_window.py` 31 → 2), let `src/app.py` drop 3 `cast()` calls and join CI's mypy scope, and leaves the arbitrary-hashable-key path as the documented seam for per-request scoping. **`Session` scope itself is deferred to Phase 3** (see the risk table below): services stay process singletons through Phases 1–2; the per-user `(session, type)` scoping is designed in Phase 3 against a real HTTP request rather than speculatively here. Done 2026-09-09.
...
| **Multi-tenancy retrofitted onto process singletons** | Phase 1.4 lands the typed-resolution half (`DependencyContainer.resolve()` generic) and leaves the arbitrary-key path as the `(session, type)` seam. The per-request `Session` scope itself is Phase 3's — built against a real HTTP request, not speculatively in Phase 1 — but *before* the API surface hardens, not after |
