# File: uadas_core/core/config.py
...
:mod:`uadas_core.core.logger` and instead writes its own bootstrap-time
...
# Bootstrap-time-only logger. Deliberately not uadas_core.core.logger.get_logger
...
def _default_config_dict() -> dict[str, Any]:
    ...
    return {
        ...
        "ai": {
            ...
            # uadas_core.core.expertise_level.ExpertiseLevel for the full set
        ...
        "ui": {
            # Milestone 29: read by src.app.Application.run to decide whether to show
...
def validate_config_structure(data: dict[str, Any]) -> None:
    ...
    :class:`~uadas_core.services.settings_service.SettingsService` runs
    before writing settings back to disk, so that a value set through
    :meth:`~uadas_core.services.settings_service.SettingsService.set` cannot
...
class AppConfig:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        :class:`~uadas_core.services.settings_service.SettingsService`, which
        ...
        :meth:`~uadas_core.services.settings_service.SettingsService.set`
