# File: uadas_core/database/database_reader.py
"""Turns a live database connection's table/query result into a Dataset.

Deliberately does **not** subclass :class:`~uadas_core.readers.base_reader.
...
dispatches on (see :mod:`uadas_core.readers.reader_registry.
...
:func:`~uadas_core.readers.reader_registry.get_reader_for_path`) and a
...
consistent with :class:`~uadas_core.database.base_connection.
    ...
    def read_table(cls, connection: BaseDatabaseConnection, table_name: str) -> Dataset:
        ...
        Args:
            connection: An already-constructed (not necessarily yet
                ...
                :class:`~uadas_core.database.base_connection.
        ...
        Raises:
            ReaderError: If the table cannot be read (propagated from
                the connection's own :class:`~uadas_core.core.exceptions.
