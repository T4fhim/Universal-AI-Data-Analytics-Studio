# File: uadas_core/database/base_connection.py
...
:mod:`uadas_core.database`'s own docstring) — the first being
:class:`~uadas_core.ai.llm_provider.BaseLLMProvider`, which this class
...
:mod:`uadas_core.database.database_reader`) never branches on which concrete
backend is active.
...
(see each subclass's own docstring for which). :class:`~uadas_core.database.
...
class BaseDatabaseConnection(ABC):
    ...
    Args:
        profile: The connection's non-secret metadata (host, port,
            ...
            :class:`~uadas_core.database.connection_profile.ConnectionProfile`
    ...
    def connect(self) -> None:
        ...
        :meth:`~uadas_core.ai.llm_provider.BaseLLMProvider`'s own tolerance
