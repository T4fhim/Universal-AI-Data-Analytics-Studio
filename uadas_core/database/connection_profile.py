# File: uadas_core/database/connection_profile.py
...
:mod:`uadas_core.services.database_connection_service` for where a password
actually lives (in-memory only, for the current session). This module
has no other behavior; it exists so
:mod:`uadas_core.core.config` (which stores a list of these, serialized to
plain dicts, under ``database.profiles``) and
:mod:`uadas_core.database.connection_registry`/every concrete connector both
...
class DatabaseType(StrEnum):
    ...
    :class:`~uadas_core.core.expertise_level.ExpertiseLevel` does: a member
...
class ConnectionProfile:
    ...
    :class:`~uadas_core.services.settings_service.SettingsService` persists as
    ...
    :meth:`~uadas_core.database.base_connection.BaseDatabaseConnection.connect`
    and held only in
