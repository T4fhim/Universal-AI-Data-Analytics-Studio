# File: uadas_core/readers/type_inference.py
...
by both :class:`~uadas_core.readers.csv_reader.CsvReader` (where a mix
typically means a malformed or inconsistently-entered CSV column) and
:class:`~uadas_core.readers.sqlite_reader.SqliteReader` (where a mix is a
...
def find_ambiguous_type_columns(dataframe: pd.DataFrame) -> list[str]:
    ...
    :class:`~uadas_core.readers.csv_reader.CsvReader`), and the history is
