# File: uadas_core/readers/xml_reader.py
...
reader raises :class:`~uadas_core.core.exceptions.ReaderError` rather than
guessing at a tabular interpretation of data that may not be tabular
at all — the same posture :class:`~uadas_core.readers.json_reader.JsonReader`
takes for JSON shapes it does not recognize as tabular (a single
non-columnar object, for instance). This is different from
:class:`~uadas_core.readers.pdf_reader.PdfReader`'s and
:class:`~uadas_core.readers.word_reader.WordReader`'s "zero tables is a valid
...
class XmlReader(BaseReader):
    ...
    def read(cls, path: Path, table_name: str | None = None) -> Dataset:
        ...
        Args:
            ...
            table_name: Accepted for interface uniformity with
                :meth:`~uadas_core.readers.base_reader.BaseReader.read`, but
                ...
                :meth:`~uadas_core.readers.csv_reader.CsvReader.read`'s
    ...
    def _element_to_record(cls, element) -> tuple[dict, list[str]]:
        ...
        :class:`~uadas_core.readers.json_reader.JsonReader` already
