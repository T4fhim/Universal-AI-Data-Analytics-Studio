# File: uadas_core/readers/powerpoint_reader.py
"""Reads tables embedded in PowerPoint (.pptx) slides into a Dataset.

Multi-table like :class:`~uadas_core.readers.excel_reader.ExcelReader`, but
...
for :class:`~uadas_core.readers.excel_reader.ExcelReader` — the same
    ...
    def read(cls, path: Path, table_name: str | None = None) -> Dataset:
        ...
        Args:
            ...
            table_name: Which table to read, by the label returned
                ...
                convenience as
                :meth:`~uadas_core.readers.excel_reader.ExcelReader.read`.
