# File: uadas_core/readers/csv_reader.py
...
class CsvReader(BaseReader):
    ...
    def read(cls, path: Path, table_name: str | None = None) -> Dataset:
        ...
        Args:
            ...
            table_name: Accepted for interface uniformity with
                :meth:`~uadas_core.readers.base_reader.BaseReader.read`
                (multi-table readers like
                :class:`~uadas_core.readers.excel_reader.ExcelReader` use
    ...
    def _find_ambiguous_type_columns(cls, dataframe: pd.DataFrame) -> list[str]:
        ...
        than here, since :class:`~uadas_core.readers.sqlite_reader.SqliteReader`
