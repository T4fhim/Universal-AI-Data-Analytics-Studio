# File: uadas_core/readers/yaml_reader.py
"""Reads YAML files into a Dataset.

Single-table (a YAML document has no worksheet/sheet concept), same
shape as :class:`~uadas_core.readers.json_reader.JsonReader` for the same
...
class YamlReader(BaseReader):
    """Reads .yaml/.yml files, the same way :class:`~uadas_core.readers.json_reader.JsonReader` reads JSON."""
    ...
    def read(cls, path: Path, table_name: str | None = None) -> Dataset:
        ...
        Args:
            ...
            table_name: Accepted for interface uniformity (see
                :meth:`~uadas_core.readers.csv_reader.CsvReader.read`'s own
