# File: uadas_core/readers/text_reader.py
...
1. **A delimited file with a ``.txt`` extension.** Plenty of
   ...
   :class:`~uadas_core.readers.csv_reader.CsvReader` uses — and if a
   consistent delimiter is detected across the file, delegates to
   :class:`~uadas_core.readers.csv_reader.CsvReader` entirely, since at that
...
class TextReader(BaseReader):
    ...
    def read(cls, path: Path, table_name: str | None = None) -> Dataset:
        ...
        Raises:
            ReaderError: If the file does not exist, is empty, or
                ...
                :class:`~uadas_core.readers.csv_reader.CsvReader`, this
    ...
    def _looks_delimited(cls, path: Path) -> bool:
        ...
        :class:`~uadas_core.readers.csv_reader.CsvReader` uses for its own
