...
mode could do for PDFs (see :mod:`~uadas_core.readers.pdf_reader`'s module
...
* **Zero tables is a valid, non-error outcome** — the same contract
  :class:`~uadas_core.readers.pdf_reader.PdfReader` implements, for the same
  reason (a prose-only Word document is not a malformed file).
* **Row 1 is assumed to be the header row** — the same assumption
  :class:`~uadas_core.readers.excel_reader.ExcelReader` and
  :class:`~uadas_core.readers.pdf_reader.PdfReader` both document, for the
...
class WordReader(BaseReader):
    ...
    :class:`~uadas_core.readers.pdf_reader.PdfReader` uses, simplified since
