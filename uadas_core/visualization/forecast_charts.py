# File: uadas_core/visualization/forecast_charts.py
"""The PREDICT stage's own chart family: overlays historical data with one or more forecasts.

Every other chart family in this package (:mod:`~uadas_core.visualization.categorical_charts`,
:mod:`~uadas_core.visualization.continuous_charts`, :mod:`~uadas_core.visualization.distribution_charts`)
...
:class:`~uadas_core.forecasting.exponential_smoothing.ForecastResult` objects (from a single
``forecast_*`` call, or from every candidate :mod:`~uadas_core.forecasting.model_comparison` fit),
not columns the user is choosing from a picker. :class:`ForecastChart` still honors
:class:`~uadas_core.visualization.base_chart.BaseChart`'s exact contract (a classmethod-only,
...
:func:`~uadas_core.visualization.base_chart.validate_columns`, it is just the *historical* slice
...
**Not registered in** :mod:`~uadas_core.visualization.chart_registry`. That registry's
...
from uadas_core.visualization.base_chart import BaseChart, validate_columns
...
def _forecast_point_estimates(result: ForecastResult) -> pd.Series:
    ...
    Mirrors :func:`~uadas_core.forecasting.model_comparison._forecast_values_as_series` exactly (a
    ...
    here rather than importing that module's private helper, since :mod:`uadas_core.visualization` has
    no other reason to depend on :mod:`~uadas_core.forecasting.model_comparison` and this one-line
    ...
    def build(
        ...
        Args:
            ...
            winner_method: If given, the candidate whose
                :attr:`~uadas_core.forecasting.exponential_smoothing.ForecastResult.method` matches
