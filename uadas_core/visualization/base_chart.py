# File: uadas_core/visualization/base_chart.py
"""The shared interface every chart type implements.

Mirrors :class:`~uadas_core.readers.base_reader.BaseReader` and
:class:`~uadas_core.cleaning.base_operation.BaseOperation`: stateless
...
def validate_columns(dataframe: pd.DataFrame, *column_names: str) -> None:
    ...
    :mod:`~uadas_core.cleaning.missing_values`'s ``_validate_columns`` helper
    ...
    def build(cls, dataframe: pd.DataFrame, **kwargs) -> go.Figure:
        ...
        Args:
            ...
            **kwargs: Chart-specific parameters (column names, chart
                ...
                :meth:`~uadas_core.cleaning.base_operation.BaseOperation.apply`'s
                same reasoning.
