# File: uadas_core/services/database_connection_service.py
...
* **Saved profiles** (:class:`~uadas_core.database.connection_profile.
  ConnectionProfile`) persist across restarts, via
  :class:`~uadas_core.services.settings_service.SettingsService`'s
  ``database.profiles`` config key (see :mod:`uadas_core.core.config`'s
  three-place schema update for this milestone). They hold no
  password.
* **Live connections** (:class:`~uadas_core.database.base_connection.
...
Registered as a bootstrap singleton alongside :class:`~uadas_core.services.
...
smaller scale: :class:`~uadas_core.services.workspace_service.WorkspaceService`
...
from uadas_core.database.connection_profile import ConnectionProfile
from uadas_core.database.connection_registry import get_connector_class
...
class DatabaseConnectionService:
    ...
    def save_profile(self, profile: ConnectionProfile) -> None:
        ...
        Matches by :attr:`~uadas_core.database.connection_profile.
    ...
    def open_connection(
        ...
        Raises:
            ServiceError: If no connector is registered for
                ...
                :meth:`~uadas_core.database.base_connection.
