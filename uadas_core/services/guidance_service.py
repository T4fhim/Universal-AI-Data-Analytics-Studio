# File: uadas_core/services/guidance_service.py
...
3. A data-quality scan built on :func:`~uadas_core.analysis.dataset_profile.profile_dataset` --
...
4. Re-ranking (not filtering) by :class:`~uadas_core.core.expertise_level.ExpertiseLevel` --
...
# through uadas_core.cleaning.operation_registry directly, not through
...
class SuggestionCategory(StrEnum):
    ...
    :class:`~uadas_core.core.expertise_level.ExpertiseLevel` does -- comparable/serializable with no
...
class Suggestion:
    ...
    Attributes:
        ...
        rationale: Why this suggestion is here -- always populated, matching this project's
            ...
            :class:`~uadas_core.visualization.chart_recommender.ChartSuggestion.reason` already
            follows).
        category: Which deterministic source produced this suggestion.
        stage: The :class:`~uadas_core.services.analysis_orchestrator_service.PipelineStage` this
...
class GuidanceService:
    ...
    def get_suggestions(
        ...
        Args:
            ...
            expertise_level: Re-ranks (never filters) the candidate set -- see this module's
                ...
                :data:`~uadas_core.core.config._default_config_dict`'s own ``ai.expertise_level``
    ...
    def _chart_suggestions(self, dataset: Dataset) -> list[Suggestion]:
        """Wrap :func:`~uadas_core.visualization.chart_recommender.recommend_charts` as CHART suggestions.
    ...
    def _data_quality_suggestions(self, dataset: Dataset) -> list[Suggestion]:
        """Turn a fresh :func:`~uadas_core.analysis.dataset_profile.profile_dataset` run into
        ...
        already-computed :class:`~uadas_core.analysis.dataset_profile.DatasetProfile` as a
