# File: uadas_core/services/report_service.py
...
shaping it into the :class:`~uadas_core.reports.report_content.ReportContent`
...
from uadas_core.reports.base_exporter import BaseReportExporter
...
# Fixed, not plugin-extensible the way uadas_core.visualization.chart_registry
# / uadas_core.cleaning.operation_registry are (milestone 12): a report format
...
# formats — uadas_core.plugins.plugin_manifest.SUPPORTED_CATEGORIES
    ...
    def build_report_content(
        ...
        Args:
            ...
            expertise_level: Recorded on the resulting
                ...
                at EXPLAIN time
                (:class:`~uadas_core.analysis.explanation.Explanation`); this
        ...
        Raises:
            ServiceError: If ``dataset_id`` is not currently loaded
                (propagated from
                :meth:`~uadas_core.services.workspace_service.
...
def _format_explanation(explanation: dict[str, Any] | None) -> str:
    ...
    Not every :class:`~uadas_core.analysis.explanation.Explanation` field is
