# File: uadas_core/reports/report_content.py
"""The shared, format-independent shape every report exporter renders from.

:class:`ReportContent` plays the same role for :mod:`uadas_core.reports` that
a dataframe plays for :mod:`uadas_core.visualization`'s
:class:`~uadas_core.visualization.base_chart.BaseChart` subclasses: it is the
one input every exporter (PDF, HTML, Word, Excel) accepts, so adding a
fifth format later means writing one new
:class:`~uadas_core.reports.base_exporter.BaseReportExporter` subclass, not
teaching :class:`~uadas_core.services.report_service.ReportService` a new
output shape. Built by :class:`~uadas_core.services.report_service.
ReportService.build_report_content`, which is the module that actually
knows how to read an :class:`~uadas_core.services.
analysis_orchestrator_service.AnalysisLog` — this module has no
dependency on that service or on :mod:`uadas_core.services` at all, keeping
the same one-way dependency direction (services depend on this
package, not the reverse) that :mod:`uadas_core.visualization` keeps with
respect to :mod:`uadas_core.services.workspace_service`.
"""
...
class ReportSection:
    ...
    AnalysisLogEntry` — :class:`~uadas_core.services.report_service.
    ...
    Attributes:
        ...
        tool_name: Which :mod:`uadas_core.ai.tool_registry` tool produced this
...
class ReportContent:
    ...
    Attributes:
        ...
        expertise_level: Which
            :class:`~uadas_core.core.expertise_level.ExpertiseLevel` this
            report was generated for. Recorded as display metadata
            only — see :meth:`~uadas_core.services.report_service.
