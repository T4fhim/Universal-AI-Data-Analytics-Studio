# File: uadas_core/reports/base_exporter.py
"""The shared interface every report format exporter implements.

Same stateless-classmethod shape as :class:`~uadas_core.readers.base_reader.
BaseReader`, :class:`~uadas_core.cleaning.base_operation.BaseOperation`, and
:class:`~uadas_core.visualization.base_chart.BaseChart`: no concrete exporter
is ever instantiated, since each is consumed as a class held in
:mod:`uadas_core.services.report_service`'s format registry, not as an
...
if TYPE_CHECKING:
    from uadas_core.reports.report_content import ReportContent
    ...
    def export(cls, report_content: ReportContent, output_path: Path, **kwargs) -> Path:
        ...
        Args:
            report_content: What to render — see
                :class:`~uadas_core.reports.report_content.ReportContent`.
            output_path: Where to write the file. The parent directory
                ...
                :class:`~uadas_core.readers.base_reader.BaseReader`'s own
