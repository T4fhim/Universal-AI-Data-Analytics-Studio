# File: uadas_core/ai/assistant_service.py
"""Runs a conversation with an LLM (Anthropic or Gemini), dispatching tool calls against the active dataset.

Delegates every provider-specific detail to :mod:`uadas_core.ai.llm_provider`
...
from uadas_core.ai.llm_provider import PendingToolCall
from uadas_core.ai.provider_rotation import (
    ProviderRotationService,
    ResolvedProviderProfile,
    _looks_like_rate_limit,
)
from uadas_core.ai.tool_registry import get_anthropic_tool_schemas, get_tool_by_name
...
class AssistantTurnResult:
    ...
    Attributes:
        ...
        new_tool_results: Any other tool call's raw return value from
            ...
            ``go.Figure`` (every :mod:`uadas_core.analysis` tool wrapped by
            :mod:`uadas_core.ai.tool_registry`: profiling, correlation,
...
class AssistantService:
    ...
    def from_provider_profiles(
        ...
        Args:
            config_profiles: The ``ai.providers`` list as stored in
                config (see :func:`uadas_core.core.config._default_config_dict`)
            ...
            expertise_level: Mirrors ``ai.expertise_level`` (milestone
                ...
                :class:`~uadas_core.core.expertise_level.ExpertiseLevel`'s
            ...
            active_provider_index: Mirrors ``ai.active_provider_index`` (milestone 29 —
                ...
                :class:`~uadas_core.ai.provider_rotation.ProviderRotationService`'s own
                ``start_index`` docstring for the out-of-range handling).

        Raises:
            ServiceError: If ``config_profiles`` is empty — mirrors
                :class:`~uadas_core.ai.provider_rotation.ProviderRotationService`'s
    ...
    def set_expertise_level(self, expertise_level: str) -> None:
        ...
        :class:`~uadas_core.ai.llm_provider.BaseLLMProvider`'s own docstring),
    ...
    def _execute_tool(
        ...
        # module's own docstring on why it flattens the underlying uadas_core.analysis dataclass) --
